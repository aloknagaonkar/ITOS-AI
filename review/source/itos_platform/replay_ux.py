...
def resolve_cutoff(timestamps: Sequence[datetime], requested: datetime) -> tuple[int, datetime]:
    eligible = [(index, stamp) for index, stamp in enumerate(timestamps) if stamp <= requested]
    if not eligible:
...
def build_outcome(analysis_candles: pd.DataFrame, future_candles: pd.DataFrame,
    ...
    future, _, _ = normalize_candles(future_candles)
    reference = None if past.empty else float(past.iloc[-1].close)
    after = future[future.timestamp >= normalize_timestamp(analysis_timestamp)]
    def at(minutes: int) -> tuple[float | None, float | None]:
        candidates = after[after.timestamp >= normalize_timestamp(analysis_timestamp) + timedelta(minutes=minutes)]
    ...
    return ReplayOutcome(
        analysis_timestamp, reference, p5, c5, p15, c15, p30, c30, end,
        ...
        not after.empty,
        () if not after.empty else ("FUTURE_DATA_UNAVAILABLE",),
