...
class DataHealthEngine(BaseEngine):
    ...
    def analyze(
        ...
        candles = snapshot.historical_candles
        candle_rows = len(candles) if hasattr(candles, "__len__") else 0
        if candle_rows <= 0:
            score -= 60
            flags.append("CANDLES_UNAVAILABLE")
            explanations.append("Intraday and historical candles are unavailable.")
        else:
            explanations.append(f"Candle history contains {candle_rows} rows.")
