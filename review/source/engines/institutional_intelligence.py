...
class PatternRecognitionEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        ...
        cycle_meta = getattr(cycle, "metadata", {})
        cycle_meta = cycle_meta if isinstance(cycle_meta, Mapping) else {}
        ...
        rvol = _num(regime.get("relative_volume"), _num(cycle_meta.get("relative_volume"), 1.0))
    ...
    def _adapt_input(market_data: DecisionContext | Mapping[str, Any]) -> Mapping[str, Any]:
        """Normalize the canonical context and legacy dictionary contract."""
        ...
        return {
            ...
            "institutional": market_data.institutional,
...
class InstitutionalRadarEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        ...
        if isinstance(metrics, InstitutionalMetrics):
            ...
            if "missing_call_oi_change" not in flags:
                call_change = metrics.oi.call_oi_change
            if "missing_put_oi_change" not in flags:
                put_change = metrics.oi.put_oi_change
