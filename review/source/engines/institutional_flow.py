...
from itos_platform.institutional_metrics import InstitutionalMetrics
...
class InstitutionalFlowEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        ...
        metrics = market_data.get("institutional_metrics")
        ...
        if isinstance(metrics, InstitutionalMetrics):
            oi_metrics = metrics.oi
            if oi_metrics.call_oi_velocity is not None and oi_metrics.put_oi_velocity is not None:
                call_change, put_change = oi_metrics.call_oi_velocity, oi_metrics.put_oi_velocity
            if oi_metrics.call_oi_acceleration is not None and oi_metrics.put_oi_acceleration is not None:
                call_accel, put_accel = oi_metrics.call_oi_acceleration, oi_metrics.put_oi_acceleration
    ...
    def _adapt_input(
        ...
        if isinstance(market_data, DecisionContext):
            return {
                ...
                "institutional_metrics": market_data.institutional_metrics,
            }
        ...
        return {
            ...
            "institutional_metrics": market_data.get("institutional_metrics"),
        }
...
class InstitutionalConfidenceEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        ...
        if volume_component is None and isinstance(metrics, InstitutionalMetrics):
            volume_component = metrics.liquidity.liquidity_score
        ...
        values = {
            ...
            "Volume Expansion": _num(volume_component, 50),
...
class EarlyWarningEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        market_data = self._adapt_input(market_data)
        recommendation_value = market_data.get("recommendation")
        recommendation = recommendation_value if isinstance(recommendation_value, Mapping) else {}
        flow = market_data.get("flow_result")
        ice = market_data.get("ice_result")
