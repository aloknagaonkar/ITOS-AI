...
def _mapping(value: Any) -> Mapping[str, Any]:
    """Normalize optional provider containers before nested access."""
    return value if isinstance(value, Mapping) else {}
...
class MarketRegimeEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        ...
        intelligence = _mapping(market_data.get("intelligence"))
        price = _mapping(intelligence.get("price"))
        flow = market_data.get("flow_result")
        cycle = market_data.get("cycle_result")
        option_result = _mapping(market_data.get("option_result"))
        summary = _mapping(option_result.get("summary"))
...
class SmartMoneyIndexEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        market_data = self._adapt_input(market_data)
        recommendation = _mapping(market_data.get("recommendation"))
...
class MarketEnergyEngine(BaseEngine):
    ...
    def analyze(self, market_data: DecisionContext | Mapping[str, Any]) -> EngineResult:
        ...
        summary = _mapping(option_result.get("summary"))
        recommendation = _mapping(market_data.get("recommendation"))
        components = _mapping(recommendation.get("component_scores"))
