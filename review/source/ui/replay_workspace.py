...
def render_replay_workspace(mode: DataMode, state: MutableMapping[str, Any],
    ...
    if state.get("historical_similarity_return_available"):
        if st.button("Back to Similarity Results", key="historical_similarity_back_from_replay"):
            state["historical_pipeline_requested_workspace"] = "HISTORICAL_SIMILARITY"
            st.rerun()
    ...
    target_stamp = state.pop("historical_similarity_replay_target", None)
    trading_date = c4.date_input("Trading date", value=target_stamp.date() if target_stamp else date.today(), key="replay_control_date")
    replay_time = c5.time_input("Replay time", value=target_stamp.time() if target_stamp else time(10, 15), step=60, key="replay_control_time")
    ...
    run_clicked = run.button("Run Replay", type="primary", use_container_width=True) or target_stamp is not None
