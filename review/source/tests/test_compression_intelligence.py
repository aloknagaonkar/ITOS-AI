...
@pytest.mark.parametrize("recent_scale,expected", [(1.2, "NO_COMPRESSION"), (.85, "EARLY_COMPRESSION"), (.65, "MODERATE_COMPRESSION"), (.35, "HIGH_COMPRESSION"), (.1, "EXTREME_COMPRESSION")])
def test_state_bands_are_behavioural(recent_scale, expected):
    result = analyze(candles(recent_scale=recent_scale), settings=CompressionIntelligenceSettings(oi_weight=0, time_weight=0))
...
def test_zero_width_and_malformed_values_degrade_safely():
    ...
    malformed = candles(); malformed.loc[0, "high"] = "bad"
    assert analyze(malformed).state in {"NO_COMPRESSION", "EARLY_COMPRESSION", "MODERATE_COMPRESSION", "HIGH_COMPRESSION", "EXTREME_COMPRESSION"}
