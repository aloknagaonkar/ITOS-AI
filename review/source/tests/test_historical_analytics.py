...
class ReadOnlyLake:
    ...
    def __init__(self, records=(), outcomes=(), *, manifest=True, settings=None,
                 manifest_engine_version="itos-18.4c-v1",
                 manifest_schema_version="intelligence-v1"):
        self.records, self.outcomes, self.queries = tuple(records), tuple(outcomes), []
        self.manifest = manifest
        self.manifest_engine_version = manifest_engine_version
    ...
    def get_manifest(self, provider, instrument, interval):
        ...
        return DatasetManifest("lake", provider, instrument, "NIFTY 50", interval, 5,
            engine_version=self.manifest_engine_version,
            intelligence_schema_version=self.manifest_schema_version,
            available_dates=days, intelligence_dates=days,
...
def test_empty_period_degrades_without_fabricated_recommendation():
    ...
    assert result.average_confidence is None and result.average_compression is None
    assert result.average_5m_change is None and result.average_eod_change is None
...
def test_empty_period_returns_shared_manifest_and_configured_engine_version():
    settings = replace(MarketLakeSettings(), engine_version="shared-engine")
    result = HistoricalAnalyticsService(ReadOnlyLake(
        settings=settings, manifest_engine_version="shared-engine"
    )).analyze(request())
    assert result.engine_versions == ("shared-engine",)


def test_test_manifest_schema_identity_is_independent_from_configured_schema():
    settings = replace(MarketLakeSettings(), intelligence_schema_version="configured-schema")
    lake = ReadOnlyLake(settings=settings, manifest_schema_version="manifest-schema")
    manifest = lake.get_manifest("upstox", "NIFTY", 1)
    assert manifest.intelligence_schema_version == "manifest-schema"
    assert lake.settings.intelligence_schema_version == "configured-schema"


def test_non_empty_stored_record_engine_versions_remain_authoritative():
    stored = record(datetime(2026, 7, 31, 10), engine="stored-engine")
    lake = ReadOnlyLake((stored,), manifest_engine_version="manifest-engine")
    result = HistoricalAnalyticsService(lake).analyze(request(engine_version="stored-engine"))
    assert result.engine_versions == ("stored-engine",)
