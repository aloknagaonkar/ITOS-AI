...
def test_jump_normalizes_naive_request_against_aware_candles():
    stamps = completed_candle_timestamps(candles(), request())
    _, resolved = resolve_cutoff(stamps, datetime(2026, 7, 30, 9, 33))
    assert resolved == datetime(2026, 7, 30, 9, 30, tzinfo=ZoneInfo("Asia/Kolkata"))


def test_jump_converts_utc_request_to_replay_timezone():
    stamps = completed_candle_timestamps(candles(), request())
    _, resolved = resolve_cutoff(stamps, datetime(2026, 7, 30, 4, 3, tzinfo=timezone.utc))
    assert resolved == datetime(2026, 7, 30, 9, 30, tzinfo=ZoneInfo("Asia/Kolkata"))


def test_jump_accepts_asia_kolkata_request_and_rejects_future_selection():
    stamps = completed_candle_timestamps(candles(), request())
    requested = datetime(2026, 7, 30, 9, 33, tzinfo=ZoneInfo("Asia/Kolkata"))
    _, resolved = resolve_cutoff(stamps, requested)
    assert resolved <= requested and resolved.time().isoformat() == "09:30:00"


def test_jump_with_no_eligible_aware_candle_uses_safe_error():
    stamps = completed_candle_timestamps(candles(), request())
    with pytest.raises(ValueError, match="No completed candle"):
        resolve_cutoff(stamps, datetime(2026, 7, 30, 3, 45, tzinfo=timezone.utc))
...
def test_outcome_future_frame_invalid_after_normalization_degrades_safely():
    future = pd.DataFrame([
        {"timestamp": "not-a-time", "open": 1, "high": 2, "low": 0, "close": 1},
    ])
    outcome = build_outcome(candles(), future, datetime(2026, 7, 30, 10))
    assert not outcome.future_data_available
    assert outcome.price_after_5m is None
    assert outcome.quality_flags == ("FUTURE_DATA_UNAVAILABLE",)


def test_outcome_accepts_non_empty_naive_future_timestamps():
    future = pd.DataFrame([
        {"timestamp": "2026-07-30 10:05", "open": 130, "high": 132, "low": 129, "close": 131},
    ])
    outcome = build_outcome(candles(), future, datetime(2026, 7, 30, 10))
    assert outcome.future_data_available and outcome.price_after_5m == 131
    assert outcome.analysis_timestamp.tzinfo == ZoneInfo("Asia/Kolkata")


def test_outcome_converts_utc_future_timestamps_to_asia_kolkata():
    future = pd.DataFrame([
        {"timestamp": datetime(2026, 7, 30, 4, 35, tzinfo=timezone.utc),
         "open": 130, "high": 132, "low": 129, "close": 131},
    ])
    outcome = build_outcome(candles(), future, datetime(2026, 7, 30, 4, 30, tzinfo=timezone.utc))
    assert outcome.future_data_available and outcome.price_after_5m == 131
    assert outcome.analysis_timestamp == datetime(2026, 7, 30, 10, tzinfo=ZoneInfo("Asia/Kolkata"))


def test_unavailable_outcome_preserves_frozen_decision_identity():
    frozen_decision = point(datetime(2026, 7, 30, 10), 73)
    outcome = build_outcome(candles(), pd.DataFrame(), datetime(2026, 7, 30, 10))
    assert not outcome.future_data_available
    assert frozen_decision == point(datetime(2026, 7, 30, 10), 73)
