...
def context(kind="quiet", **kwargs):
    ...
    location = kwargs.pop("location", SimpleNamespace(
        zone="TOP" if kind == "false_up" else "BOTTOM" if kind == "false_down" else "MIDDLE",
        support_level=97.0, resistance_level=104.0,
    ))
    volume = kwargs.pop("volume_structure", SimpleNamespace(
        volume_confirmation="DIVERGING" if kind in {"false_up", "false_down"} else "CONFIRMED",
        effort_result_state="ABSORPTION" if kind in {"false_up", "false_down"} else "BALANCED",
    ))
...
@pytest.mark.parametrize("kind, expected", [
    ("false_up", "FALSE_BREAKOUT"),
    ("false_down", "FALSE_BREAKDOWN"),
])
def test_failed_acceptance_is_classified(kind, expected):
    ...
    assert result.liquidity_sweep_detected
...
@pytest.mark.parametrize("kind, side, direction", [
    ("false_up", "ABOVE_RESISTANCE", "BEARISH_TRAP"),
    ("false_down", "BELOW_SUPPORT", "BULLISH_TRAP"),
])
def test_liquidity_sweep_stop_hunt_wick_and_direction(kind, side, direction):
    ...
    assert result.rejection_score > 0
