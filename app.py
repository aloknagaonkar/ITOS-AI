...
if should_load:
    try:
        with st.spinner("Loading option chain, candles and intelligence signals..."):
            ...
            raw_chain = client.get_option_chain(UNDERLYINGS[underlying], expiry)
            ...
            raw_candles = client.get_intraday_candles(
                UNDERLYINGS[underlying], interval=timeframe, unit="minutes"
            )

            # Retrieve enough calendar history to cover the latest two actual
            # trading sessions, including weekends and exchange holidays.
            today = date.today()
            history_from = today - timedelta(days=10)
            try:
                historical_candles = client.get_historical_candles(
                    UNDERLYINGS[underlying],
                    from_date=history_from.isoformat(),
                ...
                combined_pattern_candles = combined_pattern_candles.sort_values("timestamp").reset_index(drop=True)
            except Exception as history_exc:
                # Current-day analysis must remain available even if the dated
                # historical endpoint is temporarily unavailable.
                combined_pattern_candles = raw_candles.copy()
                st.warning(f"Historical candle pattern data is limited to the current session: {history_exc}")

            price_result = evaluate_price_action(raw_candles)
            intelligence = combine_intelligence(option_result, price_result)

            st.session_state["option_result"] = option_result
            st.session_state["intelligence"] = intelligence
            st.session_state["underlying"] = underlying
            st.session_state["expiry"] = expiry
            st.session_state["timeframe"] = timeframe
            st.session_state["historical_pattern_candles"] = combined_pattern_candles
            st.session_state["last_refresh"] = time.strftime("%H:%M:%S")

            if save_snapshots:
                store = SnapshotStore()
                snapshot_id, created = store.save_snapshot(
                    underlying, expiry, option_result, intelligence
                )
                st.session_state["snapshot_id"] = snapshot_id
                st.session_state["snapshot_created"] = created
    except UpstoxAPIError as exc:
        st.error(f"Upstox API error: {exc}")
    except Exception as exc:
        st.exception(exc)

option_result = st.session_state.get("option_result")
...
# Load stored flow history before generating the CE/PE decision. The engine still
# works without history, but confirmation becomes stronger after 15+ snapshots.
institutional = None
decision_history = pd.DataFrame()
decision_strike_history = pd.DataFrame()
if save_snapshots:
    try:
        decision_store = SnapshotStore()
        decision_underlying = st.session_state.get("underlying", underlying)
        decision_expiry = st.session_state.get("expiry", expiry)
        decision_history = decision_store.get_history(decision_underlying, decision_expiry, hours=history_hours)
        decision_strike_history = decision_store.get_strike_history(
            decision_underlying, decision_expiry, hours=history_hours
        )
        institutional = institutional_summary(decision_history, decision_strike_history)
    except Exception as exc:
        st.warning(f"Historical flow could not be included in the current recommendation: {exc}")

recommendation = build_recommendation(option_result, intelligence, institutional)

# Version 7 plugin engines. Cycle classification runs independently from the
# existing recommendation logic. Stability then evaluates whether the current
# CE/PE recommendation has remained consistent across recent refreshes.
cycle_engine = MarketCycleEngine()
cycle_result = cycle_engine.analyze({
    "option_result": option_result,
    "intelligence": intelligence,
    "institutional": institutional,
})

engine_store = SnapshotStore()
engine_underlying = st.session_state.get("underlying", underlying)
engine_expiry = st.session_state.get("expiry", expiry)
...
stability_engine = RecommendationStabilityEngine(minimum_stability=70.0)
stability_result = stability_engine.analyze({
...
# Version 7.1 institutional interpretation layer. These engines explain the
# existing recommendation; they do not bypass the established safety gates.
transition_result = PhaseTransitionEngine().analyze({"cycle_result": cycle_result})
pattern_result = PatternRecognitionEngine().analyze({
    "recommendation": recommendation, "option_result": option_result,
    "intelligence": intelligence, "institutional": institutional,
    "cycle_result": cycle_result,
})
readiness_result = TradeReadinessEngine().analyze({
    "recommendation": recommendation, "cycle_result": cycle_result,
    "stability_result": stability_result, "pattern_result": pattern_result,
})
radar_result = InstitutionalRadarEngine().analyze({
    "recommendation": recommendation, "option_result": option_result,
    "intelligence": intelligence, "institutional": institutional,
})
story_result = MarketStoryEngine().analyze({
    "recommendation": recommendation, "cycle_result": cycle_result,
    "transition_result": transition_result, "readiness_result": readiness_result,
    "radar_result": radar_result, "pattern_result": pattern_result,
})

# Version 7.5 confirmation suite. All existing engines remain unchanged; this
# layer only validates or vetoes the established CE/PE recommendation.
...
footprint_result = InstitutionalFootprintEngine().analyze({
    "option_result": option_result, "intelligence": intelligence,
    "institutional": institutional, "cycle_result": cycle_result,
})
false_breakout_result = FalseBreakoutEngine().analyze({
    "structure_result": structure_result, "candle_dna_result": candle_dna_result,
    "footprint_result": footprint_result, "cycle_result": cycle_result,
})
confirmation_result = InstitutionalConfirmationEngine().analyze({
    "recommendation": recommendation, "footprint_result": footprint_result,
    "structure_result": structure_result, "smart_candle_result": smart_candle_result,
    "candle_dna_result": candle_dna_result, "pattern_result": pattern_result,
    "cycle_result": cycle_result, "false_breakout_result": false_breakout_result,
})
recommendation["phase_transition"] = transition_result.metadata
recommendation["patterns"] = pattern_result.metadata
recommendation["trade_readiness_v71"] = readiness_result.metadata
recommendation["institutional_radar"] = radar_result.metadata
recommendation["market_story"] = story_result.metadata
recommendation["candle_dna"] = candle_dna_result.metadata
recommendation["smart_candles"] = smart_candle_result.metadata
recommendation["institutional_structures"] = structure_result.metadata
recommendation["institutional_footprint"] = footprint_result.metadata
recommendation["false_breakout"] = false_breakout_result.metadata
recommendation["institutional_confirmation"] = confirmation_result.metadata

# Version 7.6 decision workflow. These engines consume the established
# recommendation and confirmation layers; they never bypass existing gates.
decision_matrix_result = InstitutionalDecisionMatrixEngine().analyze({
    "recommendation": recommendation, "intelligence": intelligence,
    "cycle_result": cycle_result, "footprint_result": footprint_result,
    "confirmation_result": confirmation_result, "candle_dna_result": candle_dna_result,
    "pattern_result": pattern_result, "false_breakout_result": false_breakout_result,
})
...
# Hard safety gates: manipulation/compression and unstable recommendations may
# downgrade a green trigger to WATCH, but they never manufacture a BUY signal.
cycle_direction_matches = cycle_result.vote in {recommendation["side"], "WAIT"}
cycle_gate = bool(cycle_meta.get("trade_allowed", False)) and cycle_direction_matches
stability_gate = bool(stability_meta.get("passed", False))
if recommendation.get("confirmed") and not cycle_gate:
    ...
    recommendation["blockers"] = list(dict.fromkeys(recommendation.get("blockers", []) + [
        f"Market phase is {cycle_meta.get('phase', 'Unknown')}; directional expansion is required"
    ]))
if recommendation.get("confirmed") and not stability_gate:
    recommendation["confirmed"] = False
    recommendation["status"] = f"WATCH {recommendation['side']} — STABILITY DEVELOPING"
    recommendation["blockers"] = list(dict.fromkeys(recommendation.get("blockers", []) + [
        f"Recommendation stability is {stability_meta.get('stability_score', 0):.0f}%, below the 70% trigger threshold"
    ]))
if recommendation.get("confirmed") and false_breakout_result.metadata.get("blocked"):
    ...
    recommendation["blockers"] = list(dict.fromkeys(recommendation.get("blockers", []) + [
        f"False-breakout risk is {false_breakout_result.score:.0f}/100"
    ]))
if recommendation.get("confirmed") and confirmation_result.metadata.get("status") != "CONFIRMED":
    ...
    recommendation["blockers"] = list(dict.fromkeys(recommendation.get("blockers", []) + [
        f"Institutional confirmation is {confirmation_result.score:.0f}/100"
    ]))

# Keep Top-5 board colours aligned with the final gated recommendation.
...
# Version 7.7 Institutional Flow subsystem. It uses stored snapshots to
# measure flow velocity/acceleration and can only validate or downgrade the
# existing recommendation; it never creates a direction independently.
flow_result = InstitutionalFlowEngine().analyze({
    "history": decision_history, "strike_history": decision_strike_history,
    "recommendation": recommendation, "option_result": option_result,
})
ice_result = InstitutionalConfidenceEngine().analyze({
    "recommendation": recommendation, "flow_result": flow_result,
    "confirmation_result": confirmation_result, "cycle_result": cycle_result,
    "candle_dna_result": candle_dna_result, "pattern_result": pattern_result,
    "decision_matrix_result": decision_matrix_result,
})
validation_result = SignalValidationEngine().analyze({
    "recommendation": recommendation, "flow_result": flow_result,
    "ice_result": ice_result, "confirmation_result": confirmation_result,
    "false_breakout_result": false_breakout_result, "stability_result": stability_result,
})
early_warning_result = EarlyWarningEngine().analyze({
    "recommendation": recommendation, "flow_result": flow_result,
    "ice_result": ice_result, "validation_result": validation_result,
})
recommendation["institutional_flow_v77"] = flow_result.metadata
recommendation["institutional_confidence_v77"] = ice_result.metadata
recommendation["signal_validation_v77"] = validation_result.metadata
recommendation["early_warning_v77"] = early_warning_result.metadata

# Version 8.0 Core Intelligence milestone. These engines summarise the existing
# evidence into regime, smart-money, energy and opportunity-lifecycle states.
regime_result = MarketRegimeEngine().analyze({
    "option_result": option_result, "intelligence": intelligence,
    "flow_result": flow_result, "cycle_result": cycle_result,
})
smi_result = SmartMoneyIndexEngine().analyze({
    "recommendation": recommendation, "flow_result": flow_result,
    "ice_result": ice_result, "confirmation_result": confirmation_result,
    "regime_result": regime_result, "stability_result": stability_result,
    "false_breakout_result": false_breakout_result,
})
energy_result = MarketEnergyEngine().analyze({
    "recommendation": recommendation, "option_result": option_result,
    "intelligence": intelligence, "flow_result": flow_result,
})
recommendation["market_regime_v80"] = regime_result.metadata
recommendation["smart_money_index_v80"] = smi_result.metadata
recommendation["market_energy_v80"] = energy_result.metadata

# The v7.7 validator is an additional safety gate. A base trigger remains
# waiting until flow evidence has enough snapshots and at least 5/6 controls pass.
if recommendation.get("confirmed") and not validation_result.metadata.get("validated", False):
    ...
    recommendation["blockers"] = list(dict.fromkeys(recommendation.get("blockers", []) + [
        f"Version 7.7 validation passed {validation_result.metadata.get('passed', 0)} of {validation_result.metadata.get('total', 6)} controls"
    ]))
    for key in ("ce_top5", "pe_top5"):
        table = recommendation.get(key)
        if isinstance(table, pd.DataFrame) and not table.empty:
            table = table.copy()
            table["trade_state"] = "WAITING"
            recommendation[key] = table
...
try:
    confidence_store = SnapshotStore()
    confidence_underlying = st.session_state.get("underlying", underlying)
    confidence_expiry = st.session_state.get("expiry", expiry)
    if should_load:
        confidence_store.save_confidence_history(confidence_underlying, confidence_expiry, recommendation)
    confidence_history = confidence_store.get_confidence_history(confidence_underlying, confidence_expiry, hours=history_hours)
except Exception as exc:
    st.warning(f"Confidence history could not be updated: {exc}")

# Persist the lifecycle of every green/TRIGGERED candidate. Active trades are
# revalued on each refresh and completed automatically at Target 1 or stop-loss.
trade_history = pd.DataFrame()
trade_stats = {
    "total": 0, "active": 0, "success": 0, "failure": 0,
    "completed": 0, "success_rate": 0.0, "avg_pnl_percent": 0.0,
    "avg_winner": 0.0, "avg_loser": 0.0, "profit_factor": 0.0,
}
try:
    trade_store = SnapshotStore()
    tracked_underlying = st.session_state.get("underlying", underlying)
    tracked_expiry = st.session_state.get("expiry", expiry)
    if should_load:
        sync_result = trade_store.sync_trade_history(
            tracked_underlying, tracked_expiry, recommendation, option_result["chain"]
        )
        st.session_state["trade_sync_result"] = sync_result
    trade_history = trade_store.get_trade_history(tracked_underlying, tracked_expiry)
    trade_stats = trade_store.trade_statistics(tracked_underlying, tracked_expiry)
except Exception as exc:
    st.warning(f"Trade history could not be updated: {exc}")

trade_plan_result = None

data_health_result = DataHealthEngine().analyze({
    "option_result": option_result,
    "intelligence": intelligence,
    "recommendation": recommendation,
...
# The detailed trade planner is initialized later because its position-sizing
# controls live inside the Live Market Intelligence tab.  The hero card can
# safely build from the ranked recommendation first; AITradeEngine falls back
# to the best-contract entry, stop and target fields when no planner result is
# available yet.
ai_trade_opportunity = AITradeEngine().build(
    recommendation=recommendation,
...
with tab_live:
    st.caption(
        f"{st.session_state.get('underlying', underlying)} • "
        f"Expiry {st.session_state.get('expiry', expiry)} • "
        f"{st.session_state.get('timeframe', timeframe)}-minute candles • "
        f"Last refresh {st.session_state.get('last_refresh', '-')}"
    )
    ...
    with st.expander("Historical Pattern Intelligence — Last 2 Trading Days", expanded=True):
        if historical_patterns.empty:
            ...
            st.download_button(
                ...
                file_name=f"{engine_underlying.replace(' ', '_').lower()}_pattern_intelligence_v752.csv",
                mime="text/csv",
            )
...
with tab_live:
    ...
    try:
        historical_long = engine_store.get_history(engine_underlying, engine_expiry, hours=24 * 45)
    except Exception:
    ...
    try:
        if should_load:
            engine_store.save_playbook_history(engine_underlying, engine_expiry, playbook_result)
            engine_store.save_decision_audit(
                engine_underlying, engine_expiry, recommendation, regime_result, smi_result,
                energy_result, opportunity_result, similarity_result, playbook_result, report_result,
            )
        decision_audit_history = engine_store.get_decision_audit(engine_underlying, engine_expiry)
        playbook_history = engine_store.get_playbook_history(engine_underlying, engine_expiry, hours=24 * 7)
    except Exception as exc:
...
with tab_explorer:
    ...
    if trade_history.empty:
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"{tracked_underlying.replace(' ', '_')}_{tracked_expiry}_trade_history.csv",
            mime="text/csv",
        )
...
with tab_live:
    ...
    fig = make_market_chart(
        ...
        max_pain=s["max_pain"],
        title=f"{st.session_state.get('underlying', underlying)} — Native Upstox Intraday Chart",
    )
    st.plotly_chart(fig, use_container_width=True)
...
with tab_explorer:
    ...
    if save_snapshots:
        store = SnapshotStore()
        stored_underlying = st.session_state.get("underlying", underlying)
        stored_expiry = st.session_state.get("expiry", expiry)
        history = store.get_history(stored_underlying, stored_expiry, hours=history_hours)
        strike_history = store.get_strike_history(stored_underlying, stored_expiry, hours=history_hours)
        institutional = institutional_summary(history, strike_history)
    
        h1, h2, h3 = st.columns(3)
        h1.metric("Stored snapshots", store.count_snapshots(stored_underlying, stored_expiry))
