...
def _json_value(value: Any, *, _seen: set[int] | None = None) -> Any:
    """Return deterministic JSON data, excluding runtime-only values safely.

    Dashboard results contain a mixture of typed intelligence objects and runtime
    collaborators.  Unknown objects are never stringified because their repr often
    contains memory addresses or credentials.
    """
    ...
    if isinstance(value, Enum): return _json_value(value.value, _seen=seen)
    ...
    if isinstance(value, (tuple, list)):
        seen.add(identity)
        result = [_json_value(item, _seen=seen) for item in value]
        seen.discard(identity)
        return [item for item in result if item is not _EXCLUDED]
    if is_dataclass(value): return _json_value(asdict(value), _seen=seen)
    if hasattr(value, "item"):
        try: return _json_value(value.item(), _seen=seen)
...
def _excluded_key(key: Any) -> bool:
    lowered = str(key).lower().replace("-", "_")
    return lowered.startswith("_") or lowered in _RUNTIME_KEYS or _secret(lowered)
...
def serialize_dashboard_result(result: Any, request: ReplayRequest, provider: str, settings: MarketLakeSettings) -> HistoricalIntelligenceRecord:
    ...
    serial = _json_value({key: value for key, value in values.items() if not _excluded_key(key)})
