...
class HistoricalAnalyticsService:
    ...
    def _aggregate(self, request: HistoricalAnalyticsRequest, records: tuple[HistoricalIntelligenceRecord, ...],
        ...
        average_or_zero = lambda values: _average(values) if records else 0.0
        median_or_zero = lambda values: _median(values) if records else 0.0
        ...
        result = HistoricalAnalyticsResult(
            ...
            _counts(compression_states), _counts(manipulation_states), average_or_zero(confidence), median_or_zero(confidence),
            ...
            average_or_zero(compression_scores), median_or_zero(compression_scores), max(compression_scores, default=None),
            ...
            {"average_volume_strength": average_or_zero([_value(r.values, "volume_structure.volume_strength", "volume_strength") for r in records]),
            ...
            {"average": average_or_zero(compression_scores), "median": median_or_zero(compression_scores),
            ...
            {"bullish_count": evidence_bull, "bullish_percentage": _pct(evidence_bull, len(records)),
             ...
             "average_evidence_quality": average_or_zero([_value(r.values, "institutional_evidence.quality") for r in records])},
            ...
            horizon(5) if records else 0.0, horizon(15) if records else 0.0, horizon(30) if records else 0.0,
            _average(eod) if records else 0.0,
            _average([o.maximum_favourable_excursion for o in outcomes]) if records else 0.0,
            _average([o.maximum_adverse_excursion for o in outcomes]) if records else 0.0,
