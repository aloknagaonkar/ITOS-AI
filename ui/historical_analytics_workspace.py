...
def render_historical_analytics_workspace(underlyings: Mapping[str, str], *, provider: str = "upstox",
    ...
    if active_run is not None:
        retry = st.columns(3)
        if retry[0].button("Cancel After Current Date", key="historical_pipeline_cancel_after_date"):
            st.session_state["historical_pipeline_cancel_requested"] = True
        if retry[1].button("Retry Failed Dates", key="historical_pipeline_retry_failed",
        ...
        if retry[2].button("Retry Index Only", key="historical_pipeline_retry_index",
