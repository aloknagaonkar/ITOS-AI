...
@pytest.mark.parametrize("engine", [MarketRegimeEngine, SmartMoneyIndexEngine, MarketEnergyEngine, EarlyWarningEngine])
@pytest.mark.parametrize("malformed", [None, 17, {}, {"recommendation": None, "intelligence": 4,
                                                       "option_result": [], "flow_result": _result(metadata=None)}])
...
def test_missing_validation_cannot_promote_blocked_recommendation():
    data = _legacy()
    data["recommendation"].update(confirmed=False, status="WAIT CE — BLOCKED")
    data["validation_result"] = None
    result = EarlyWarningEngine().analyze(_context(data))
    assert result.vote == "WAIT"
    assert result.metadata["state"] == "EARLY CE ALERT"
    assert result.metadata["informational_only"] is True
    assert result.metadata["preparation"] == "Prepare contract and wait for validation"
